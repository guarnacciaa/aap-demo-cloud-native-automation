...
plain `extra_vars` (unlike `smtp_host`/`smtp_port`/`notification_to`/`notification_from`).
They are instead stored on the **SMTP Credentials** custom credential (`smtp_credential_name` in
